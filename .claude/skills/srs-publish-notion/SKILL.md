...
把 `{batch}-SRS/output/requirements-{batch}.md` 發佈到 Notion workspace 的沙箱 toggle
《需求分析》內的「需求說明文件3-B」child page。
...
`需求說明文件3-B` Notion page 在每次 publish 後**必須**處於下列狀態。任何
違反此契約的人工編輯都會在下次 publish 時被覆寫：

1. **Flat 結構** — 頁面**不應有任何 child page**。9 個需求的所有內容皆 inline
   於本頁。若 publish 前偵測到既有 child page，會在 Step 6 自動刪除後再寫入。
...
- Notion target：讀 `.claude/notion-mapping.json` 的 `batches[{batch}]`，
  其中包含 `parent_page_id`、`sandbox.child_page_id`、`sandbox.toggle_label` 等
...
1. 在父頁面（`.claude/notion-mapping.json` 中的 `parent_page_id`）手動建立
   一個 toggle heading，標題為「《需求分析》」
2. 在該 toggle 內建立一個 child page，標題為「需求說明文件3-B」
...
```json
{
  "batches": {
    "3-B": {
      "parent_page_id": "20cd6e30-3b5c-80c4-a9f5-c041bfdca150",
      "parent_page_url": "https://www.notion.so/ewill-software/20cd6e303b5c80c4a9f5c041bfdca150",
      "sandbox": {
        "toggle_label": "《需求分析》",
...
決定 batch ID 後，從 `batches[{batch}]` 讀取對應設定。
...
1. Read `.claude/notion-mapping.json`
...
2. fetch 結果會是 Notion-flavored markdown，toggle 與 child page 大致長這樣：
   ```
   <details>
   <summary>《需求分析》</summary>
   <page url="https://www.notion.so/ewill-software/xxx">需求說明文件3-B</page>
   </details>
   ```
3. 找到 `<summary>` 含「《需求分析》」的 `<details>` 區塊，在其中抓出
   `<page url="...">需求說明文件3-B</page>` tag，提取 URL 與 page ID
...
**Read** `.claude/skills/notion-doc-writing/SKILL.md`（專案本地已有此 skill），
必要時進一步 Read 下列支援檔案：
- `.claude/skills/notion-doc-writing/markdown-gotchas.md` — 完整陷阱清單
- `.claude/skills/notion-doc-writing/block-selection.md` — block type 選用
- `.claude/skills/notion-doc-writing/property-conventions.md` — 屬性命名

確認以下規則內化：

- Title 不在 content 第一行（我們源檔有 `# 需求說明文件 3-B`，須剝除）
- Heading 最深 H4（我們的六大章節 + 子層若超過要降級或折 toggle）
- 多行 quote 用 `<br>` 連接
- Tab 縮排
...
1. **剝除最外層 H1**
   `# 需求說明文件 3-B` → 移除（title 由 properties 管）
...
### 偵測到的需求
- R0044, R0050, R0051, R0052, R0053, R0054, R0056, R0058, R0060
（共 9 份；如果有缺漏，這裡會提示）

### 將執行的 Notion 操作
- ⚠️ 刪除既有 child page: N 個
  · 需求說明文件3-B/R0044_點檢作業功能
  · 需求說明文件3-B/R0050_報修作業功能
  · ...（列出所有將被刪除的 child page）
...
實務上 9 個需求的 SRS 應該在 900 block 以內，fallback 罕用。
...
- R0044/R0044_單身宿舍_財產點檢表_中文.png — 原說明「…」
- R0050/R0050_...png — …
（共 N 張）
