#!/usr/bin/env python3
"""
Markdown → Notion Block JSON 轉換器

依據 Notion API 官方 Block Reference 實作，支援以下 Block 類型：
- heading_1 / heading_2 / heading_3（含 is_toggleable、color）
- paragraph（含 color）
- bulleted_list_item（含巢狀 children）
- numbered_list_item（含巢狀 children）
- to_do（含 checked）
- toggle（含 children）
- quote（含多行合併、color）
...
用法：
    python3 convert.py input.md                    # 輸出 JSON 到 stdout
    python3 convert.py input.md -o output.json     # 輸出到檔案
    python3 convert.py input.md --pretty            # 格式化輸出
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Markdown → Notion Block JSON 轉換器'
    )
